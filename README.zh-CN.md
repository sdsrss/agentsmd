...
- **npm 包 / `npx`:** 一次性 `npx @sdsrs/agentsmd install`,或版本固定的全局 `agentsmd` CLI。
...
如果你想要一次性 `npx` 运行、事后不留全局安装,或想要一个由 npm 版本固定的
`agentsmd` CLI,用这条路径。包内自带 `agentsmd` CLI,所以**你永远不需要
`npm explore`**。它以 `@sdsrs/agentsmd` 作用域发布(npm 拒绝了无作用域的
`agentsmd` 名,认为它与已有包过于相似);安装进 Codex 的运行时足迹名仍是 `agentsmd`。

用 `npx` 跑一次(不做全局安装):

```bash
npx @sdsrs/agentsmd install     # 把 agentsmd 并入 $CODEX_HOME
npx @sdsrs/agentsmd status      # 确认注册了什么
npx @sdsrs/agentsmd doctor      # 健康检查
```
...
`status` · `doctor` · `audit` · `rules`。裸调用 `agentsmd`(或不带命令的
`npx @sdsrs/agentsmd`)只打印帮助,不安装任何东西。
...
> **插件路径接了什么、没接什么。** Codex 会从插件 bundle 自动管理 agentsmd 的
> **hooks**。完整安装的其余部分——把核心规范注入 `~/.codex/AGENTS.md`、设置
> `config.toml` 的 `[features] hooks = true`、以及迁移旧的 codexmd 安装——由脚本
> 安装器完成,**而非**插件安装。要拿到完整体验,加插件后再跑一次安装器:
>
> ```bash
...
```
bin/         npm CLI 入口——`agentsmd` / `npx @sdsrs/agentsmd` 对 scripts/ 的 dispatcher
...
`npx @sdsrs/agentsmd@latest install`);用 `agentsmd uninstall` 后
