...
function usage() {
  ...
  '  exception <add|list|rm|prune>   Register reviewed §8 false-positive exceptions in this repo\'s .agentsmd/exceptions.json (fingerprint + expiry; no inline bypass tokens).',
    '  install [--json] [--degraded] [--profile=full]   Install/update the complete spec, hooks, and skills in $CODEX_HOME (~/.codex), including the managed global AGENTS.md block.',
    '  update [--json] [--degraded] [--profile=full]    Alias for install — existing standalone installs remain updateable.',
