...
function usage() {
  ...
  '  exception <add|list|rm|prune>   Register reviewed §8 false-positive exceptions in this repo\'s .agentsmd/exceptions.json (fingerprint + expiry; no inline bypass tokens).',
    '  install [--json] [--degraded] [--allow-dual-surface]   Install/update agentsmd into $CODEX_HOME (~/.codex). Skips a fresh standalone surface when the enabled plugin is present unless dual delivery is explicit.',
    '  update [--json] [--degraded] [--allow-dual-surface]    Alias for install — existing standalone installs remain updateable.',
