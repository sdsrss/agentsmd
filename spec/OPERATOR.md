...
The convention-adoption layer (`analyze --write` stamps `@conv-<dim>` anchors → `convention-cite-scan` records cites → `analyze --adoption` reports them) is advisory and self-measuring, and structurally independent of the `§*` enforcement/demote loop. It earns its keep only if the operator actually reads its output and prunes dead dimensions — otherwise the stamped block is pure context rent. The review discipline:

- **Never prune off a thin window.** Wait until a project's adoption ledger clears the `MIN_EXPOSURE_SESSIONS` distinct-session floor of real (untagged) sessions; below it, `analyze --adoption` reads `insufficient-exposure`, not a verdict. Then run `node scripts/analyze.js --adoption --project=<repo> --days=<window>`.
- **Prune on the negative only** (per §O7's Goodhart caveat): a dimension at 0 cites in an adequately-exposed window is a prune candidate — a human deletes that heading from the project's `AGENTS.md` (never automatic). A high cite count certifies nothing.
- **Baseline (2026-07-05).** Ledger = 20 total events, **0 `cite` events** → `insufficient-exposure`. The layer shipped in v2.9.0 the same day; no project has accumulated adoption signal yet. Do not prune. Re-evaluate once real projects have run `analyze --write` and logged sessions past the exposure floor.
