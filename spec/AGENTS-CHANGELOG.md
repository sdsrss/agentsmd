...
The shared version moves in lockstep with a package minor adding multi-stack
project detection, evidence-gated command facts, and stratified gather
sampling (R4-05, closes Gate E). `spec_version` → v4.16.0. Detail: repository
`CHANGELOG.md`.
...
Core and extended rule text, the manifest rules, and all anchors are unchanged.
The shared version moves in lockstep with a package minor that executes
journal-based crash recovery at every lifecycle entry and journals uninstall
(R2-03). `spec_version` → v4.12.0. Detail: repository `CHANGELOG.md`.
...
Core and extended rule text, the manifest rules, and all anchors are unchanged.
The shared version moves in lockstep with a package minor that serializes all
mutating lifecycle operations per `$CODEX_HOME` behind a cross-process lock
(R2-01). `spec_version` → v4.8.0. Detail: repository `CHANGELOG.md`.
