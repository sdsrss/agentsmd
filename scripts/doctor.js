...
function doctor() {
  ...
  const configuration = inspectStandaloneConfiguration({
    surfaceStatus,
    cfg,
    hooksParseable: rawHooks === null || rawHooks.trim() === '' || H.parseHooksConfig(rawHooks) !== null,
    registeredHooks,
    manifestRaw: read(P.manifestPath()),
  });
  checks.push(...configuration.checks);
  const { manifestInstalled, manifest } = configuration;
