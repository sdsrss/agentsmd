...
Key fields: `agentsmdHooksRegistered` (should be 7), `otherTenantHooksPreserved` (OMX / other plugins agentsmd left untouched — proves independence), `codexHooksFlag`, `specBlockInAgentsMd`, `telemetryRows`. If `installed` is false, run `node scripts/install.js` (a §5-hard action — confirm with the user first).
