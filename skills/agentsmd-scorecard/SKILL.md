...
- When an operator supplies an external evidence directory and a source checkout is available, use `npm run captures:inventory -- --root=/absolute/evidence-dir --json` there. This explicit bounded read cannot write/import evidence; use the existing candidate/binding flags only after inspecting the located records. Never scan home/config directories to guess an evidence location. Inventory hashes do not prove current applicability, and formal SLO captures do not replace the packaged reference baseline.
