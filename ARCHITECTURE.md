# codexmd — Architecture & Build Plan

Codex 版编程全局规范**系统**（不只是一份规范文本）的架构设计与分阶段实施计划。参照 `/mnt/data_ssd/dev/projects/claudemd` 的成熟三层形态，适配 Codex CLI 的真实能力面。本文件是设计正典；实施进度见 `tasks/codexmd-build.md`。
...
```
L3  命令层    Codex skills（dir + SKILL.md）：codexmd-audit / doctor / rules / status
...
spec/AGENTS*.md 的 (HARD) 规则
  └─ spec/hard-rules.json           机器可读镜像（每条规则的 section_anchor + enforcement + codex_hook_event）
      └─ hooks/*.sh 强制             命中/阻断/fail-open 时 hook_record
          └─ ~/.codex/logs/codexmd.jsonl   append-only 遥测（ts/hook/event/project/session_id/spec_section/extra）
              └─ scripts/audit.js    bySection 聚合
                  └─ 升降级信号       0 命中的 always-on 规则 → 降级到 extended；高频命中 → 证明值回 core
```

- 遥测写入器移植 claudemd `hooks/lib/rule-hits.sh`：改日志路径 `~/.claude/logs/claudemd.jsonl` → `~/.codex/logs/codexmd.jsonl`，project 字段编码沿用 `tr -c 'a-zA-Z0-9-' '-'`，保留 size-capped rotation。
...
**codexmd 是一个独立的 Codex 插件，与 oh-my-codex(OMX) 完全解耦。** 用户可能**根本没装 OMX**，codexmd 必须能独立安装运行；装了 OMX 时，两者的**安装/更新/卸载互不冲突、互不影响**。这是 HARD 不变式，不是「尽量」。

**codexmd 绝不**：假设 OMX 存在 · 读取/依赖/修改 OMX（或任何其他租户）的任何条目 · 在共享文件里重排或触碰非自己的内容。**codexmd 只管自己的条目**，靠命令路径标记 `/codexmd/` 唯一识别（绝不会撞上 OMX 的 `codex-native-hook.js` 或其他插件）。

**装卸语义**（照搬 OMX `codex-hooks.js` 生产验证的 merge/remove 模式，用 codexmd 自己的标记）：
...
| state / log | 自有路径 `~/.codex/.codexmd-state/`、`~/.codex/logs/codexmd.jsonl` | 无碰撞 |

**打包形态**：标准 `.codex-plugin/plugin.json`（`name:"codexmd"` + `skills:"./skills/"` + 携带 `hooks.json`）。若 Codex 对插件携带的 `hooks.json` 自动装配（figma 插件已携带 hooks.json，倾向支持）→ 装卸隔离由插件系统免费提供；若不自动 → codexmd 附带 postinstall 跑上面的标记式 merge。**两条路都满足独立性**。

**定位**：OMX（若在）是编排框架，codexmd 是纪律/执行力层——互补而非竞争；但 codexmd **不依赖** OMX。
...
agentsmd/
  ...
  hooks/                     L1 强制层（Phase 1-2）
    hooks.json               codexmd 的 hook 条目（供安装器 append 进 ~/.codex/hooks.json）
  ...
  tasks/                     机器本地工作状态（codexmd-build.md）
```
...
| **3** | L2 脚本（install/status/audit/doctor/toggle）+ **标记式 merge/remove 安装器**（§5，只增删 `/codexmd/` 自有条目）+ 自有 manifest + kill-switch；首次 **re-AUTH** 触碰 live hooks.json/config.toml/AGENTS.md | 是（re-AUTH） | 待办 |
...
- **[Phase 3]** codexmd 规范部署形态：`AGENTS.override.md` vs 独立 include vs 与 OMX AGENTS.md 协商——不覆盖前提下的最优挂载点。
