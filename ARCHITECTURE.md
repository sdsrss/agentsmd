...
- **中断恢复 = journal + 逐步复核**：恢复计划不能授权后续出现的事务外修改。每步重新核对目标，文件写入/删除绑定预期快照，目录交换先验证恢复源与备份；步骤完成后再次检查全部目标。冲突保留新字节和 pending journal，不回滚覆盖外部写入。恢复清理同时验证 journal 记录的路径与内容哈希；未登记的 stage 内容保留并诊断，旧 journal 无法证明的额外内容也不按目录名删除。此机制沿用上述 POSIX 非原子比较边界。
...
- **缓存热路径**：每份 hook 用一次成功的 stat 调用取得 manifest mtime/size，并用一次 jq 调用验证单个缓存对象的数值 schema、plugin root、manifest key 和 selected surface 枚举。只有有效且选择另一物理面的缓存才让当前副本退出；多 JSON 文档、错误类型、未知选择、过期 key 或探测失败均保留两份运行。GNU stat 失败后尝试 BSD stat；失败输出不能作为 key。该检查复用既有缓存协议，不替代完整 health inspector，也不证明宿主只执行一次。
